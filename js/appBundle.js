 ...
 * CLI version: 2.12.0
 *
 * Generated: Tue, 08 Aug 2023 10:03:04 GMT
 */
...
var APP_com_metrological_app_vod_example = (function () {
  ...
  const transition = function (target, value) {
    let resetDur = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 0.3;
  ...
  class MenuItem extends Lightning.Component {
    ...
    _updateFocusAnimation() {
      let bool = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : this._selected;
      ...
      return {
        CutOut: {
          ...
          TMDB: {
            ...
            x: 1840,
            mount: 1,
            src: Utils.asset('images/tmdb-logo.png')
          }
        }
      };
    }
  ...
  class Player extends Page {
    ...
    _startOverlayTimeout() {
      ...
      this._overlayTimeout = setTimeout(() => {
        ...
        if (this.active && this._progressAnimation.isActive()) {
          ...
          this._showEventMessage("Mahantesh is Out of OOO");
        }
      }, 3000);
    }
    ...
    _update() {
      ...
      this.patch({
        Placeholder: {
          texture: Img(backdrop).contain(1920, 1080)
        },
        Title: {
          text: title
        }
    ...
    _updatePlayButton() {
      let toPlay = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : true;
    ...
    _showEventMessage(message) {
      ...
      eventMessage.setSmooth('alpha', 0, {
        delay: 1.5,
        duration: 0.2
      });
    }
    _handleEnter() {
      ...
      this._showEventMessage("handle ".concat(currentButton.action, " button action"));
    }
    _captureKey() {
  ...
  var $inject_process_env = {"APP_API_KEY":"e47d2c0a2fafa05a047ef1814da59153"};
  ...
  const _params = {
    'api_key': $inject_process_env.APP_API_KEY,
    include_adult: false,
    include_video: true,
    region: 'NL'
  };
  const _executeRequest = function (config) {
    let retryCounter = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 0;
    const {
      url,
      target,
      body,
  ...
  const _fetchPageData = function (lists) {
    let itemParams = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
    const calls = lists.map(_ref2 => {
      let {
        path,
        params
      } = _ref2;
  ...
  const getSearchResults = query => {
    return getRequest({
      ...
      response.results = response.results.filter(_ref3 => {
        let {
          media_type,
          poster_path
        } = _ref3;
        return media_type !== 'person' && poster_path !== null;
      });
  ...
  const getMoviesPage = () => {
    ...
    return _fetchPageData([{
      ...
      path: 'discover/movie',
      params: {
        'release_date.gte': futureDate.gte,
        'release_date.lte': futureDate.lte,
        'with_release_type': 3
      },
      title: 'Upcoming'
    }, {
      path: 'discover/movie',
      params: {
        'release_date.gte': pastDate.gte,
        'release_date.lte': pastDate.lte,
        'with_release_type': 3
      },
      title: 'In Theaters'
    }], {
  ...
  const getSeriesPage = () => {
    const currentDate = new Date();
    const formatCurrentDate = "".concat(_nomalizeDatePart(currentDate.getDay()), "-").concat(_nomalizeDatePart(currentDate.getMonth() + 1), "-").concat(currentDate.getFullYear());
    return _fetchPageData([{
      path: 'discover/tv',
      title: 'Most Popular'
    }, {
      ...
      path: 'discover/tv',
      params: {
        'release_date.gte': formatCurrentDate,
        'release_date.lte': formatCurrentDate
      },
      title: 'Airing Today'
    }], {
      media_type: 'tv'
  ...
  const getDetailPage = (mediaType, mediaId) => {
    return getRequest({
      target: "".concat(mediaType, "/").concat(mediaId),
      params: {
        append_to_response: 'episode_groups,images'
      }
    }).then(response => {
      return {
  ...
  const _futureDate = date => {
    ...
    return {
      gte: "".concat(_nomalizeDatePart(currentYear), "-").concat(_nomalizeDatePart(currentMonth + 1), "-").concat(currentDay < 22 ? 22 : currentDay),
      lte: "".concat(_nomalizeDatePart(futureYear), "-").concat(_nomalizeDatePart(futureMonth + 1), "-14")
    };
  };
  const _pastDate = date => {
    ...
    return {
      lte: "".concat(_nomalizeDatePart(currentYear), "-").concat(_nomalizeDatePart(currentMonth), "-").concat(currentDay),
      gte: "".concat(_nomalizeDatePart(pastYear), "-").concat(_nomalizeDatePart(pastMonth), "-").concat(pastDay)
    };
  };
  ...
  const createItemCollection = function (items) {
    let media_type = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'tv';
  ...
  const applyItemModel = item => {
    ...
    return {
      ...
      poster: "http://image.tmdb.org/t/p/w185/".concat(poster_path),
      large_poster: "http://image.tmdb.org/t/p/w300/".concat(poster_path),
      backdrop: "http://image.tmdb.org/t/p/original/".concat(backdrop_path)
    };
  };
  const applyPlayerModel = item => {
    ...
    return {
      id,
      title: media_type === 'tv' ? name : title,
      backdrop: "http://image.tmdb.org/t/p/original/".concat(backdropPath)
    };
  };
  ...
  class App extends Router.App {
    static getFonts() {
      return [{
        ...
        family: 'MediumItalic',
        url: Utils.asset('fonts/montserrat/Montserrat-MediumItalic.ttf')
      }];
    }
    static _template() {
      return {
